class Solution:
    def maxArea(self, height: list[int]) -> int:
        ...
        for i in range(1, len(height)):
            ...
            a = min(height[start], height[i]) * (i-start)
            b = min(height[end], height[i]) * (i-end)
            tmp = max(a, b, max_water)
            if tmp == a:
                # if max_water < min(height[start], height[i]) * (i-start):
                print(
                    f'\t s -> {min(height[start], height[i]) * (i-start)} > { max_water}')
                ...
                print(
                    f'\t e -> {min(height[end], height[i]) * (i-end)} > { max_water}')
